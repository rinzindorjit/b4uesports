...
let cachedPiNetworkService: any = null;
let cachedPricingService: any = null;
let cachedEmailService: any = null;

// Optimized import functions with caching
...
async function getPiNetworkService() {
  // Return cached module if available
  if (cachedPiNetworkService) {
    return cachedPiNetworkService;
  }
  
  try {
    // Try to import from the server directory (works in Vercel)
    const piNetworkModule = await import("../server/services/pi-network.js");
    cachedPiNetworkService = piNetworkModule.piNetworkService;
    return cachedPiNetworkService;
  } catch (error) {
    try {
      // Additional fallback for direct relative import (works in some cases)
      const piNetworkModule = await import("./services/pi-network.js");
      cachedPiNetworkService = piNetworkModule.piNetworkService;
      return cachedPiNetworkService;
    } catch (fallbackError) {
      // Final fallback - rethrow original error
...
async function getPricingService() {
  ...
  try {
    ...
    cachedPricingService = pricingModule.pricingService;
    return cachedPricingService;
  } catch (error) {
    try {
      // Additional fallback for direct relative import (works in some cases)
      const pricingModule = await import("./services/pricing.js");
      cachedPricingService = pricingModule.pricingService;
      return cachedPricingService;
    } catch (fallbackError) {
      // Final fallback - rethrow original error
...
async function getEmailService() {
  ...
  try {
    ...
    cachedEmailService = emailModule.sendPurchaseConfirmationEmail;
    return cachedEmailService;
  } catch (error) {
    try {
      // Additional fallback for direct relative import (works in some cases)
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      app.post('/api/pi-auth', async (req: Request, res: Response) => {
        // Check if we're accessing from localhost in development
