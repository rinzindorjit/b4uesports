...
var DatabaseStorage = class {
  ...
  async getLatestPiPrice() {
    const [priceRecord] = await db.select().from(piPriceHistory).orderBy(desc(piPriceHistory.timestamp)).limit(1);
    return priceRecord || void 0;
  }
  async getAnalytics() {
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/pi-balance", async (req, res) => {
    try {
      ...
      const mockBalance = {
        balance: Math.random() * 1e3 + 100,
        // Random balance between 100-1100 Pi
        currency: "\u03C0",
